...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        ...
        switch (indexPath.row) {
            case 0: return [self createSwitchCellWithTitle:@"Read Receipt Ghost" Detail:@"Don't mark DMs as read — messages stay unread on server" Key:@"ghost_read_receipt"];
            case 1: return [self createSwitchCellWithTitle:@"Profile View Ghost" Detail:@"Become invisible when visiting someone's profile" Key:@"ghost_profile_view"];
            case 2: return [self createSwitchCellWithTitle:@"Hide Online Status" Detail:@"Always appear offline to other users" Key:@"ghost_online_status"];
            case 3: return [self createSwitchCellWithTitle:@"Hide Typing Indicator" Detail:@"Don't show 'typing...' while composing messages" Key:@"ghost_typing"];
        }
    }
